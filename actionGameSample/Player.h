...
public:
	...
	float				next_x ;
	float				next_y ;
	float			    w = 24.0f;						// プレーヤーの当たり判定幅
	float			    h = 32.0f;						// プレーヤーの当たり判定高さ
	...
	int					imgp_Atk[5];
	float				p_xOffset;					// 描画位置修正
	float				damagecont;					//プレーヤーが次にダメージを受けるまでの秒
	int					EfektAnimNum;
	...
	const float			Modification = 16.0f;				//描画位置修正
	const float			efektModification_x = 48.0f;
	const float			efektModification_y = 32.0f;
