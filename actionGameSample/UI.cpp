...
void UI::DrawUI(Rule *rule, Player *player,BitmapText text)
{
	...
	case STATE_GAME:
		...
		if(player->hp>=20)
		{
			DrawBox(0, 0, 0 + WD + player->hpba, 20, GetColor(0, 255, 255), FALSE);//メーターの枠を描画
			DrawBox(0, 0, 0 + (WD + player->hpba)* player->hp / player->hp_max, 20, GetColor(0, 255, 255), TRUE);//メーターの中身を描画
		}
		
		else
		{
			DrawBox(0, 0, 0 + WD + player->hpba, 20, GetColor(255, 0, 0), FALSE);//メーターの枠を描画
			DrawBox(0, 0, 0 + (WD + player->hpba) * player->hp / player->hp_max, 20, GetColor(255, 0, 0), TRUE);//メーターの中身を描画
		}
        if (player->hp >= 51)
        {
            DrawBox(0, 0, 0 + WD + player->hpba, 20, GetColor(0, 150, 255), FALSE);//メーターの枠を描画
            DrawBox(0, 0, 0 + (WD + player->hpba)* player->hp / player->hp_max, 20, GetColor(0, 150, 255), TRUE);//メーターの中身を描画
        }
        if (player->hp >= 61)
        {
            DrawBox(0, 0, 0 + WD + player->hpba, 20, GetColor(0, 100, 255), FALSE);//メーターの枠を描画
            DrawBox(0, 0, 0 + (WD + player->hpba)* player->hp / player->hp_max, 20, GetColor(0, 100, 255), TRUE);//メーターの中身を描画

        }
        if (player->hp >= 71)
        {
            DrawBox(0, 0, 0 + WD + player->hpba, 20, GetColor(0, 50, 255), FALSE);//メーターの枠を描画
            DrawBox(0, 0, 0 + (WD + player->hpba)* player->hp / player->hp_max, 20, GetColor(0, 50, 255), TRUE);//メーターの中身を描画
        }
        if (player->hp >= 80)
        {
            DrawBox(0, 0, 0 + WD + player->hpba, 20, GetColor(0, 0, 255), FALSE);//メーターの枠を描画
            DrawBox(0, 0, 0 + (WD + player->hpba)* player->hp / player->hp_max, 20, GetColor(0, 0, 255), TRUE);//メーターの中身を描画
        }
		break;
